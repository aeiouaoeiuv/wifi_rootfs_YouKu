...
function error404(message)
	...
	--if not luci.util.copcall(luci.template.render, "/err/404.html") then
...
function error500(message)
	...
	--else
	--	require("luci.template")
	--	if not luci.util.copcall(luci.template.render, "/err/500.html", {message=message}) then
	--		luci.http.prepare_content("text/plain")
	--		luci.http.write(message)
	--	end
	--end
...
function http_request_log(request, tag)
  ...
  if requestUri and tag and type(tag) == "string" then
	...
	--logger.flog(6,tag..":"..uriInfo[1])
	--if uriInfo[2] then
	    --logger.flog(7,uriInfo[2])
	--end
  end
end
...
function authenticator.htmlauth(validator, accs, default)
  ...
  local logintype = luci.http.formvalue("type")
  local errortype = ""

  if logintype and pass and logintype == "0"  then
     if  YKsecure.checkAdminPwd(pass) then
        return  "admin"
	 else
	    errortype = "pwd"
     end
  elseif logintype and pass and logintype == "1"  then
    if  YKsecure.checkSNPwd(pass) then
        return  "admin"
	else
	    errortype = "sn"
    end
	...
	require("luci.template")
	context.path = {}
	luci.template.render("sysauth", {duser=default, fuser=errortype})
	return false
...
function dispatch(request)
	...
	if (c and c.index) or not track.notemplate then
		...
		local luciver = commonFunc.getyoukuvertion()
	...
	if not _noauthAccessAllowed(track.flag) and track.sysauth then
...
--- contain the table in model 
function create_model_index()
	local path = luci.util.libpath() .. "/model/tbl/"
	local suff = { ".lua", ".lua.gz" }

	--if luci.util.copcall(require, "luci.fastindex") then
	--	createindex_fastindex(path, suff)
	--else
    create_model_index_plain(path, suff)
	--end
end

function create_model_index_plain(path, suffixes)
	local tbls = { }
	for _, suffix in ipairs(suffixes) do
		nixio.util.consume((fs.glob(path .. "*" .. suffix)), tbls)
		nixio.util.consume((fs.glob(path .. "*/*" .. suffix)), tbls)
	end

	if mod_indexcache then
		local cachedate = fs.stat(mod_indexcache, "mtime")
		if cachedate then
			local realdate = 0
			for _, obj in ipairs(tbls) do
			...
			if cachedate > realdate then
				assert(
					sys.process.info("uid") == fs.stat(mod_indexcache, "uid")
					and fs.stat(mod_indexcache, "modestr") == "rw-------",
					"Fatal: mod_Indexcache is not sane!"
				)

				index = loadfile(mod_indexcache)()
				return index
			end
		end
	end

	for i,c in ipairs(tbls) do
		local modname = "luci.model.tbl." .. c:sub(#path+1, #c):gsub("/", ".")
		for _, suffix in ipairs(suffixes) do
		...
		assert(mod ~= true,
		       "Invalid model table file found\n" ..
	...
	if mod_indexcache then
		local f = nixio.open(mod_indexcache, "w", 600)
