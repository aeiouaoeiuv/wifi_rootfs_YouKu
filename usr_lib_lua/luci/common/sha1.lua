...
local function from_hex(hex)
   ...
   base.assert(base.type(hex) == 'string')
   base.assert(hex:match('^[0123456789abcdefABCDEF]+$'))
   base.assert(#hex == 8)
...
function sha1(msg)
   ...
   base.assert(base.type(msg) == 'string')
   base.assert(#msg < 0x7FFFFFFF) -- have no idea what would happen if it were large
...
local function hex_to_binary(hex)
   return hex:gsub('..', function(hexval)
                            return string.char(base.tonumber(hexval, 16))
                         end)
end
...
function hmac_sha1(key, text)
   base.assert(base.type(key)  == 'string', "key passed to hmac_sha1 should be a string")
   base.assert(base.type(text) == 'string', "text passed to hmac_sha1 should be a string")
