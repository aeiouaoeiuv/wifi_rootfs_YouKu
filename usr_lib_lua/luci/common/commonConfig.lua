module ("luci.common.commonConfig", package.seeall)

ROUTER_QQ = ""
ROUTER_WX = "ytluyoubao"
...
GET_ROUTER_VERSION = "cat /tmp/youku/ver/main"
...
ACC_CHECK_CONF = "/usr/sbin/check_svc.sh check accmon"
SN_YOUKU_EXEC = "sn_youku r"

FORK_RESTART_WIFI = "wifi downup & >/dev/null"
FORK_RESET_ALL = "sleep 5 ; jffs2reset -y ; reboot & >/dev/null 2>/dev/null"
FORK_RESTART_ROUTER = "sleep 5 ; reboot & >/dev/null 2>/dev/null"
FORK_RESTART_DNSMASQ = "/etc/init.d/dnsmasq restart"
FORK_RESET_DATACEACH = "lua check_luci.lua 1 & "
RESTART_MAC_FILTER = "/bin/sh /etc/firewall.macfilter"
RESTART_GUEST_MODE = "/etc/init.d/network restart ; sleep 1 ; /etc/init.d/dnsmasq restart & "
GUEST_MODE_UP = "ifconfig ra1 up ; wifi downup & "
GUEST_MODE_DOWN = "ifconfig ra1 down ; wifi downup & "
...
DO_UPGRADE = "rm -rf /tmp/updater_persent; updater 1 >/tmp/updater_persent 2>/dev/null &"
GET_DOWNLOADFILE_STATUS = "tail -n 20 /tmp/updater_persent"

ROM_BIN_FILE = "/tmp/image.bin"
VERIFY_IMAGE = "sysupgrade -T "..ROM_BIN_FILE
FLASH_IMAGE = "sleep 5 ; sysupgrade "..ROM_BIN_FILE.." &"
